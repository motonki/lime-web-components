...
release:
	@# Bumps and publishes.
	docker run --rm --user=root -e CI -e GH_USERNAME -e GH_TOKEN -e NPM_TOKEN -e GIT_AUTHOR_NAME -e GIT_AUTHOR_EMAIL -e GIT_COMMITTER_NAME -e GIT_COMMITTER_EMAIL -w /lime $(DOCKER_IMAGE) /bin/bash -c "git remote rm origin && git remote add origin \"https://${GH_TOKEN}@github.com/lundalogik/lime-web-components.git\" && git remote -v && npm run build && npx lerna version --loglevel=DEBUG --allow-branch=$(BRANCH) --conventional-commits --create-release=github --yes && npx lerna --loglevel=DEBUG publish from-git --registry=http://npm.lundalogik.com:4873/"
