...
```tsx
import { CurrentLimeobject } from 'lime-web-component-interfaces/state';
...
> **Note!** There is an issue at the moment where the state decorators does not function properly unless the `componentWillLoad` and `componentDidUnload` lifecycle hooks are implemented in the component. The methods do not have to do anything, they just have to exist.
